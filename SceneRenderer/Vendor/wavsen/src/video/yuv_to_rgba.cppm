...
enum class ConvertTarget : std::uint32_t
{
    BridgeForeign = 0,
    SampledLocal  = 1,
};
...
public:
    ...
    static auto create(VkInstance instance, VkPhysicalDevice phys, VkDevice device,
                       std::uint32_t queue_family, VkQueue queue, std::uint32_t max_w,
                       std::uint32_t max_h) -> rstd::Result<std::unique_ptr<YuvToRgba>, Error>;

    auto convert_nv12(VkImage dst, std::uint32_t dst_w, std::uint32_t dst_h,
                      const std::uint8_t* nv12, std::size_t nv12_size, const ColorMatrix& cm)
    ...
    auto convert_av_vk_frame(const VkFrameImports& imports, VkImage dst, std::uint32_t dst_w,
                             std::uint32_t dst_h, const ColorMatrix& cm)
        -> rstd::Result<int, Error>;
    auto convert_av_vk_frame(const VkFrameImports& imports, VkImage dst, std::uint32_t dst_w,
    ...
    auto convert_drm_prime(const DrmFrameView& drm, VkImage dst, std::uint32_t dst_w,
                           std::uint32_t dst_h, const ColorMatrix& cm) -> rstd::Result<int, Error>;
    auto convert_drm_prime(const DrmFrameView& drm, VkImage dst, std::uint32_t dst_w,
                           std::uint32_t dst_h, const ColorMatrix& cm, ConvertTarget target)
        -> rstd::Result<int, Error>;
...
private:
    ...
    bool init(VkInstance instance, VkPhysicalDevice phys, VkDevice device,
              std::uint32_t queue_family, VkQueue queue, std::uint32_t max_w, std::uint32_t max_h,
              Error* err);
    int  convert_nv12_(VkImage dst, std::uint32_t dst_w, std::uint32_t dst_h,
                       const std::uint8_t* nv12, std::size_t nv12_size, const ColorMatrix& cm,
                       ConvertTarget target, Error* err);
    int  convert_av_vk_frame_(const VkFrameImports& imports, VkImage dst, std::uint32_t dst_w,
                              std::uint32_t dst_h, const ColorMatrix& cm, ConvertTarget target,
                              Error* err);
    ...
    VkDeviceMemory last_drm_memories_[4] {
