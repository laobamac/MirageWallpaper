// WallpaperApp_win32.cpp
//
// Windows entry point for SceneRenderer wallpaper.
// Based on WallpaperApp.cpp with platform-specific sections replaced:
//   - Crash handler:  execinfo → StackWalk64 / CaptureStackBackTrace
//   - ParseDouble:    strtod_l → _strtod_l
//   - Env:            MVK_CONFIG_* removed (no MoltenVK on Windows)
//   - Frame callback: LiveMetalFrameCallback → LiveFrameCallback (RGBA path)
//   - Host API:       WinDesktopHost.h instead of MacDesktopHost.h
//
// All other logic (arg parsing, SceneWallpaper runtime, ControlChannel)
// is preserved verbatim.

#include "../../Sources/SceneRenderer/Host/win32/WinDesktopHost.h"
...
// Windows-specific headers (replace execinfo.h / xlocale.h)
...
// --- Crash handler: Windows SEH / StackWalk64 (replaces execinfo backtrace) ---
namespace {

LONG WINAPI Win32CrashHandler(EXCEPTION_POINTERS* exception_info) {
    ...
    std::cerr << "[SceneWallpaper] CRASH: 0x"
              << std::hex << exception_info->ExceptionRecord->ExceptionCode
              << std::dec << " (" << name << ")" << std::endl;

    // Capture stack trace via CaptureStackBackTrace (simpler, no dbghelp init).
    void* callstack[64];
    USHORT frames = CaptureStackBackTrace(0, 64, callstack, nullptr);
    std::cerr << "[SceneWallpaper] Backtrace (" << frames << " frames):" << std::endl;

    // Resolve symbols if dbghelp is initialized.
    HANDLE process = GetCurrentProcess();
    SymInitialize(process, nullptr, TRUE);

    SYMBOL_INFO* symbol = reinterpret_cast<SYMBOL_INFO*>(
        calloc(1, sizeof(SYMBOL_INFO) + MAX_SYM_NAME));
    if (symbol != nullptr) {
        ...
        for (USHORT i = 0; i < frames; ++i) {
            DWORD64 addr = reinterpret_cast<DWORD64>(callstack[i]);
            DWORD   disp = 0;
        ...
        for (USHORT i = 0; i < frames; ++i) {
            std::cerr << "  " << i << ": 0x" << std::hex
                      << reinterpret_cast<DWORD64>(callstack[i]) << std::dec
                      << std::endl;
        }
    }
...
// Also handle CRT signals (SIGABRT, SIGSEGV, etc.) by mapping to SEH.
void SignalHandler(int sig) {
    ...
    std::cerr << "[SceneWallpaper] CRASH: signal " << sig
              << " (" << name << ")" << std::endl;
    ...
    for (USHORT i = 0; i < frames; ++i) {
        std::cerr << "  " << i << ": 0x" << std::hex
                  << reinterpret_cast<DWORD64>(callstack[i]) << std::dec << std::endl;
    }
    std::cerr << std::flush;
    std::_Exit(128 + sig);
}
...
// --- Forward-declare: LiveFrameCallback registration (from engine) ---
// This is the non-Metal, CPU readback path that works on Windows.
...
// Windows replacement for strtod_l:
// Uses _create_locale + _strtod_l (MSVC CRT).
bool ParseDouble(std::string_view text, double& out) {
    ...
    const double value = c_locale != nullptr
                             ? _strtod_l(value_text.c_str(), &parsed_end, c_locale)
...
// --- Frame callback (Windows: RGBA path, not Metal) ---

void LiveRGBAFrameCallback(const uint8_t* rgba,
                            uint32_t width, uint32_t height,
                            void* userdata) {
    auto* state = static_cast<AppState*>(userdata);
    if (state == nullptr || state->desktop == nullptr) return;
    SceneRendererMacDesktopPresent(state->desktop, rgba, width, height);
}

// --- Input callbacks (unchanged from macOS) ---
...
int main(int argc, char** argv) {
    ...
    // NOTE: No MoltenVK on Windows — remove macOS env vars.
    // setenv("MVK_CONFIG_PRESENT_WITH_COMMAND_BUFFER", "1", 0);   // DELETED
    // setenv("MVK_CONFIG_SYNCHRONOUS_QUEUE_SUBMITS", "1", 0);     // DELETED
    ...
    if (state.desktop == nullptr) {
        std::cerr << "Failed to create Windows desktop wallpaper host\n";
        return 1;
    }
    ...
    // Windows: use LiveFrameCallback (RGBA CPU readback) instead of
    // LiveMetalFrameCallback (Metal texture export, not available on Windows).
    SceneRendererSetLiveFrameCallback(LiveRGBAFrameCallback, &state);
    ...
    // Wait for Vulkan init and scene load.
    ...
    // Control channel: disabled on Windows for M1 (stdin handling differs).
    // Enable in M4 after adapting ReadFile(STD_INPUT_HANDLE).
