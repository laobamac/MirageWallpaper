...
// Bring in VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR for the
// portability flag in Instance::Create below. The sr.vulkan module
// re-exports the types but not the bit-flag enumerators.
...
VkResult LoadLibrary(utils::DynamicLibrary& dlib, vvk::InstanceDispatch& dld) {
    ...
    dlib = DynamicLibrary("libvulkan.1.dylib");
    if (! dlib.IsOpen()) dlib = DynamicLibrary("libvulkan.dylib");
    if (! dlib.IsOpen()) dlib = DynamicLibrary("/usr/local/lib/libvulkan.1.dylib");
    if (! dlib.IsOpen()) dlib = DynamicLibrary("/opt/homebrew/lib/libvulkan.1.dylib");
