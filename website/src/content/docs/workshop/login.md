...
下载创意工坊内容需要登录一个 Steam 账号。登录在[设置向导](/workshop/setup-wizard/)的第 3 步完成，之后由 SteamCMD 维护会话。
...
- 返回上一步也会安全地取消正在进行的登录。
- 遇到问题时可重试，或参考[故障排查](/workshop/troubleshooting/)。

## 隐私

- 密码只用于本机 SteamCMD 登录。
- 诊断日志会对密码、API Key、令牌等敏感字段做脱敏（替换为 `[已隐藏]`）。
- 更多细节见 [SteamCMD 与 Steam 登录](/workshop/steamcmd/)。
