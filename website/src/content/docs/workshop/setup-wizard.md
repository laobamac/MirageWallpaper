...
安装过程中可以取消。只有检测到或安装完成后才能进入下一步。SteamCMD 的存放位置和隔离机制见 [SteamCMD 与 Steam 登录](/MirageWallpaper/workshop/steamcmd/)。
...
登录日志会实时显示，遇到问题可据此排查。登录成功后才能进入最后一步。关于隐私与凭据处理，见[SteamCMD 与 Steam 登录](/MirageWallpaper/workshop/steamcmd/)。

## 第 4 步：完成

确认设置完成，Mirage 会记住你的用户名以便下次复用会话。之后即可正常[浏览](/MirageWallpaper/workshop/browse/)和[下载](/MirageWallpaper/workshop/download/)创意工坊内容。
...
:::tip[建议先配置 API Key]
向导主要处理 SteamCMD 与登录。浏览用的 Steam Web API 有一个所有用户共享的内置 Key，容易繁忙。建议额外填写你自己的 [Steam Web API Key](/MirageWallpaper/workshop/api-key/)，浏览体验会更稳定。
:::
