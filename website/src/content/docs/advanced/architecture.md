...
- [性能设置](/MirageWallpaper/settings/performance/)中的「停止（释放内存）」等策略，实际就是终止对应的渲染进程以释放资源。
...
- 构建这些组件见[从源码构建](/MirageWallpaper/advanced/build/)。
- 单独调试某个渲染器见[调试渲染器](/MirageWallpaper/advanced/debug-renderers/)。
- 各类型壁纸的能力对比见[壁纸类型](/MirageWallpaper/formats/wallpaper-types/)。
