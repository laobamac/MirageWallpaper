...
点击壁纸卡片将其选中，Mirage 会加载它的预览和属性面板。选中状态下你可以调整[播放参数与自定义属性](/wallpapers/playback/)，再决定是否正式应用。
...
默认情况下壁纸应用在主显示器。若要让当前壁纸铺满所有显示器，可使用**覆盖到所有显示器**（主窗口或[菜单栏](/wallpapers/menubar/)均可触发）。想为不同显示器分别指定壁纸，见[多显示器](/wallpapers/displays/)。
...
应用未受信任来源的**网页壁纸**时，Mirage 会先弹出安全提示，因为网页壁纸会执行 JavaScript。请仅信任来源可靠的网页壁纸。详见[网页壁纸安全](/settings/web-safety/)。
