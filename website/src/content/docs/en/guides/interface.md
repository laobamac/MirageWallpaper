...
description: Get to know the top tabs, wallpaper library, filter panel, and entry buttons of Mirage's main window.
---
...
Under the "Installed" tab, a toolbar runs across the top of the content area:

- **Search box**: filter wallpapers by name and other keywords.
- **Filter**: expands the filter panel (see below).
- **Refresh**: appears when "Auto refresh" is enabled in Settings, and re-scans the wallpaper directory.
- **Sort direction**: toggles between ascending and descending.
- **Sort by**: sorts by **name**, **rating**, or **file size**.

## Filter panel

Clicking "Filter" expands a sidebar with a "Reset filters" button at the top, followed by a few groups of conditions:

**Show only** (multiple selections allowed)
...
Covers the common Wallpaper Engine categories, such as Abstract, Anime, Cyberpunk, Fantasy, Game, Landscape, Nature, Sci-Fi, Pixel Art, MMD, and more. You can "Select all" or "Clear" and then check them freely.
