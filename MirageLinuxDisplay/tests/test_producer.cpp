...
int main(void) {
    ...
    callbacks.on_output_config = on_config;
    callbacks.on_retire_buffers = on_retire;
    callbacks.on_pointer_enter = on_enter;
    callbacks.on_pointer_leave = on_leave;
    ...
    info.stable_output_id = "test-output";
    info.kind = "scene";
    info.formats = &format;
    info.format_count = 1U;
    assert(md_producer_begin_connected_fd(producer, broker.client_fd, "test-producer", "0.2",
                                          &info) == MD_OK);
    broker.client_fd = -1;
    for (;;) {
        int32_t handshake = md_producer_advance_handshake(producer);
