...
/* The import diagnostics API must be NULL-safe and always yield a phrase. */
static void test_diagnostics(void) {
    assert(md_vk_importer_last_error(NULL) == NULL);
    for (int stage = static_cast<int>(MD_VK_IMPORT_STAGE_NONE);
         stage <= static_cast<int>(MD_VK_IMPORT_STAGE_SYNC_IMPORT); ++stage) {
        assert(md_vk_import_stage_string(static_cast<md_vk_import_stage_t>(stage)) != NULL);
    }
}

/* The DMA-BUF import probe must reject NULL handles and never return NULL text. */
static void test_dma_buf_probe_null_args(void) {
    md_vk_dma_buf_import_state_t state = MD_VK_DMA_BUF_IMPORT_UNAVAILABLE;
    assert(md_vk_query_dma_buf_import_support(VK_NULL_HANDLE, VK_NULL_HANDLE, &state,
                                              NULL, 0) == MD_ERR_INVALID);
    assert(md_vk_query_dma_buf_import_support(VK_NULL_HANDLE, VK_NULL_HANDLE, NULL,
                                              NULL, 0) == MD_ERR_INVALID);
    assert(md_vk_dma_buf_import_state_string(MD_VK_DMA_BUF_IMPORT_OK) != NULL);
    assert(md_vk_dma_buf_import_state_string(MD_VK_DMA_BUF_IMPORT_DRIVER_UNSUPPORTED) != NULL);
    assert(md_vk_dma_buf_import_state_string(MD_VK_DMA_BUF_IMPORT_DEVICE_NOT_ENABLED) != NULL);
    assert(md_vk_dma_buf_import_state_string(MD_VK_DMA_BUF_IMPORT_UNAVAILABLE) != NULL);
}
...
static void test_real_device_format_queries(void) {
    ...
    for (uint32_t physical_device_index = 0; physical_device_index < device_count;
        ...
        // Probe the DMA-BUF import support on a minimal extension-less device.
        // Any probe state is valid here: a driver without the external-memory
        // extensions yields UNAVAILABLE, an extension-less device that still
        // resolves the entry point yields DEVICE_NOT_ENABLED (or OK).
        float priority = 1.0f;
